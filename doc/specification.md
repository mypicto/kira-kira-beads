...
1. 上段：**問題エリア**
   ...
   * `easy` / `normal`：横一列 5 マス
   * `hard`：3 行 × 5 列（合計 15 マス）
   * フェーズにより内容が変化
2. 中段：**状態／結果エリア**
...
* スロットサイズは縮小し、横スクロールなしで `easy/normal` は 5 マス、`hard` は 3 行 × 5 マスが収まるようにする。
* グリッド → 縦積み：
...
| ID           | 要素名      | 種別       | 説明                             |
| ------------ | -------- | -------- | ------------------------------ |
| SLOT_ROW     | スロット列    | コンテナ     | 難易度ごとにスロット数（5 or 15）。`hard` は3行5列。 |
...
| BTN_LEVEL    | 難易度選択ドロップダウン | セレクト   | easy / normal / hard を選択         |
...
* `numSlots`: 15（3 行 × 5 列で表示）
* `numBeadsInProblem`: 15（全スロットにビーズを配置）
* `showDurationMs`: 500
* `colors`: NORMAL と同じ
* `shapes`: NORMAL と同じ
* `sizes`: NORMAL と同じ

### 6.3 難易度選択（初期難易度は NORMAL）

* ラウンド表示の隣のドロップダウン（BTN_LEVEL）で easy / normal / hard を選択（初期は normal）。
* 切替時：
...
* 左側：正解パターン
  ...
  * ラベル：お手本アイコン
  * スロット表示は難易度に応じて 5 マス or 15 マス（3×5）
...
* 難易度選択：
  ...
  * BTN_LEVEL（ドロップダウン）で easy / normal / hard を選ぶと `idle` に戻し、次回の出題を新設定で行う。
* 次へ：
