...
* 右下の BTN_LEVEL をクリックすると EASY / NORMAL をトグル。
* 切替時：
...
簡易版として、「正解スロットにおけるユーザーのビーズの有無」と「正しい位置への配置」を評価する。
...
※ より厳密な「距離評価（隣のマスなら 0.75 など）」は将来拡張とし、同位置一致／不一致のみ。
...
* 決定：
  ...
  * BTN_OK をクリックすると採点処理を行い、`result` へ遷移。
* リセット：
